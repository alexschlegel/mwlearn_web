...
def process_request(request):
	...
	if 'user' in data and 'action' in data and 'key' in data:
		user = data['user']
...
def read(user, result):
	...
	data = mrt_data.find_one({'user': user}, {key: True})
...
def write(user, result, value):
	...
	mrt_data.update({'user': user}, {"$set": {key: value}}, upsert=True)
